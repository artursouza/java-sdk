...
class Command {
  ...
  public void run() throws InterruptedException, IOException {
    ...
    final Thread stdoutReader = new Thread(() -> {
      try {
        try (InputStream stdin = this.process.getInputStream()) {
          try (InputStreamReader isr = new InputStreamReader(stdin, "UTF-8")) {
    ...
    if (!finished.tryAcquire(SUCCESS_WAIT_TIMEOUT_MINUTES, TimeUnit.MINUTES)) {
      throw new IllegalStateException("Command did not complete: " + command);
    }
    if (!success.get()) {
